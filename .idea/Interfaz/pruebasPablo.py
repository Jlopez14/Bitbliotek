...
class Login(App):
    ...
    def on_register_press(self, instance):
        ...
        # Crear el diseño de superposición con la imagen de fondo
        background_layout = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(500))
        background_image = Image(source='verdesitofondito.jpg', allow_stretch=True, keep_ratio=False, y=dp(-50))  # Ajustar la posición en el eje Y
        background_layout.add_widget(background_image)

        # Añadir el contenido original (registration_window) sobre la imagen de fondo
        overlay_layout = BoxLayout(orientation='vertical')
        overlay_layout.add_widget(background_layout)
        overlay_layout.add_widget(registration_window)

        popup = Popup(title='Registro de usuario', content=overlay_layout, size_hint=(None, None), size=(dp(500), dp(500)))
        popup.open()
